    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    for idx, (source_image, test_video, random_seed, size, step) in tqdm(
        ...
        if args.rank == 0:
            ...
            video_name = Path(args.image_path).stem
            source_name = Path(args.video_path).stem

            save_videos_grid(samples_per_video[-1:], f"{savedir}/{source_name}_{video_name}.mp4")
