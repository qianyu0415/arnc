...
class WebApp():
    def __init__(self, debug_mode=False):
        ...
        self.back_color = [0.858, 0.858, 0.858, 1.0]#背景颜色，接近白色的灰色

        #使用Gradio库进行用户界面的构建
        self.args_input = {}  # for gr.components only 空字典，参数由web界面提供（可能）
        self.gr_charac = list(CHARAC_TRIGGER_WORD.keys())#将前面的角色字典的列表赋给self.gr_charac
        self.gr_action = list(ACTION_TRIGGER_WORD.keys())#将前面的动作字典的列表赋给self.gr_action
        self.charac_id = 'Curry'

        # fun fact: google analytics doesn't work in this space currently
        #将用户的交互数据发送到Google Analytics，从而分析用户行为，优化用户体验
...
if __name__ == "__main__":
    ...
    demo.queue().launch(share=True, server_name='0.0.0.0', max_threads=80)
