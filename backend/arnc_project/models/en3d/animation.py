...
os.environ['MODELSCOPE_CACHE']='./models/en3d/cache/hub'
os.environ['MODELSCOPE_DATA']='./models/en3d/cache/hub'


def parse_args( ):
    parser = argparse.ArgumentParser()
    parser.add_argument("--charac_id", type=str, default='200000')
    parser.add_argument("--pose_id", type=str)
...
model_dir = './models/en3d/damo/cv_3d-human-animation'
blender_file = os.path.join(model_dir, '3D-assets', 'blender-3.1.2-linux-x64.tar.xz')
blender_path = os.path.join(model_dir, '3D-assets', 'blender-3.1.2-linux-x64', 'blender')
...
if __name__ == "__main__":
    ...
    input = {'dataset_id': 'damo/3DHuman_synthetic_dataset', # character dataset, fixed
             'case_id': args.charac_id, # character id, choose one from character dataset
             'action_dataset': 'damo/3DHuman_action_dataset', # action dataset, fixed
             'action': args.pose_id, # action id, choose one from action dataset
