...
if __name__ == '__main__':
    ...
    # 将RGBA转换为BGRA，OpenCV使用BGR顺序
    img_bgra = cv2.cvtColor(img_np, cv2.COLOR_RGBA2BGRA)
    
    # 使用OpenCV缩放图像
    if height_percent >= 1:
        resized_img_bgra = cv2.resize(img_bgra, (new_width, new_height), interpolation=cv2.INTER_AREA)
    else:
        resized_img_bgra = cv2.resize(img_bgra, (new_width, new_height), interpolation=cv2.INTER_CUBIC)
    
    # 将缩放后的BGRA图像转换回RGBA格式
    resized_img_rgba = cv2.cvtColor(resized_img_bgra, cv2.COLOR_BGRA2RGBA)
    
    # 将NumPy数组转换回Pillow图像
    resized_image = Image.fromarray(resized_img_rgba)
    ...
    for frame in range(start_frame, end_frame + 1):
        ...
        # 将NumPy数组转换回Pillow图像
        img = Image.fromarray(resized_img_rgba)
